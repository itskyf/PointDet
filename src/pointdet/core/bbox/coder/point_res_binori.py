...
class PointResBinOriCoder:
    def __init__(self, bin_size: int):
        super().__init__()
        self.bin_size = bin_size
        self.code_size = 6 + 2 * self.bin_size
        self.h_bin_inter = np.pi / self.bin_size
        self.bin_inter = 2 * self.h_bin_inter
    ...
    def encode_torch(
        ...
        """
        Args:
            points: (num_fg_points, 3) [x, y, z]
            gt_boxes: (num_fg_points, 7 + C) [x, y, z, dx, dy, dz, heading]
            gt_labels: (num_fg_points) [0, num_classes - 1]
        Returns:
            box_coding: (num_fg_points, 8 + C)
        """
        ...
        # norm to [-1, 1]
        bin_res = ((rg + np.pi) - (bin_id * self.bin_inter + self.h_bin_inter)) / self.h_bin_inter
    ...
    def decode_torch(
        ...
        bin_id = torch.max(bin_id, dim=-1).indices
