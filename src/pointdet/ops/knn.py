...
class _knn_points(Function):
    ...
    def backward(ctx, grad_dists, grad_idx):
        ...
        if grad_dists.dtype is not torch.float32:
            grad_dists = grad_dists.float()
        if p1.dtype is not torch.float32:
            p1 = p1.float()
        if p2.dtype is not torch.float32:
            p2 = p2.float()
        grad_p1, grad_p2 = _C.knn_points_backward(p1, p2, lengths1, lengths2, idx, norm, grad_dists)
...
def knn_points(
    ...
    if lengths1 is None:
        lengths1 = torch.full((p1.size(0),), P1, dtype=torch.int64, device=p1.device)
    if lengths2 is None:
        lengths2 = torch.full((p1.size(0),), P2, dtype=torch.int64, device=p1.device)
...
def knn_gather(x: torch.Tensor, idx: torch.Tensor, lengths: Optional[torch.Tensor] = None):
    ...
    if lengths is None:
        lengths = torch.full((x.size(0),), M, dtype=torch.int64, device=x.device)
