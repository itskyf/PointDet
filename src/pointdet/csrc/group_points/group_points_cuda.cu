...
__global__ void group_points_kernel(const T *p_points, const int *__restrict__ p_indices, T *p_out,
                                    int batch_size, int feat_dims, int num_feats, int num_groups,
                                    int num_neighbors) {
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < totals; i += blockDim.x * gridDim.x) {
    if (bs_idx >= batch_size || c_idx >= feat_dims) {
      return;
    }
    ...
    const int in_idx = bs_idx * feat_dims * num_feats + c_idx * num_feats + p_indices[0];
    const int out_idx = (bs_idx * feat_dims + c_idx) * totals + pt_idx * num_neighbors + sample_idx;
...
__global__ void group_points_backward_kernel(const T *p_grad_grouped,
                                             ...
                                             const int batch_size, const int feat_dims,
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < totals; i += blockDim.x * gridDim.x) {
    ...
    const int pt_idx = i / num_neighbors, sample_idx = i % num_neighbors;
    p_grad_grouped += (bs_idx * feat_dims + c_idx) * totals + pt_idx * num_neighbors + sample_idx;
    p_indices += bs_idx * totals + pt_idx * num_neighbors + sample_idx;
    atomicAdd(p_grad_feats + (bs_idx * feat_dims + c_idx) * num_feats + p_indices[0],
              p_grad_grouped[0]);
  }
}
...
at::Tensor GroupPointsBackwardCuda(const at::Tensor grad_grouped, const at::Tensor indices,
  ...
  dim3 blocks(GetBlockSize(num_groups * num_neighbors), feat_dims, batch_size);
  dim3 threads(kThreadsPerBlock);
  auto grad_feats = at::zeros({batch_size, feat_dims, num_feats}, grad_grouped.options());
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_grouped.scalar_type(), "group_points_backward_kernel", [&] {
        group_points_backward_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
            ...
            grad_feats.data_ptr<scalar_t>(), batch_size, feat_dims, num_feats, num_groups,
            num_neighbors);
      });
  AT_CUDA_CHECK(cudaGetLastError());
