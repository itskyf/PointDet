...
def array_converter(
    apply_to: tuple[str, ...], to_torch=True, template_arg_name_=None, recover=True
):
        ...
        def new_func(*args, **kwargs):
            ...
            if template_arg_name not in all_arg_names:
                raise ValueError(
                    ...
                    f"argument list of function {func.__name__}"
                )

            # inspect apply_to
            for arg_to_apply in apply_to:
                if arg_to_apply not in all_arg_names:
                    raise ValueError(f"{arg_to_apply} is not " f"an argument of {func.__name__}")
            ...
            for i, arg_value in enumerate(named_args):
                new_args.append(
                    converter.convert(input_array=arg_value, target_type=target_type)
            ...
            def recursive_recover(input_data):
                ...
                if isinstance(input_data, dict):
                    ...
                    for key, val in input_data.items():
                        new_data[key] = recursive_recover(val)
                    return new_data
                if isinstance(input_data, (torch.Tensor, np.ndarray)):
                    return converter.recover(input_data)
                return input_data

            return recursive_recover(return_values) if recover else return_values
...
class ArrayConverter:
    SUPPORTED_NON_ARRAY_TYPES: tuple = (
        int,
        float,
        np.int8,
        np.int16,
    ...
    def set_template(self, array):
        ...
        if isinstance(array, np.ndarray):
            ...
            if array.dtype not in self.SUPPORTED_NON_ARRAY_TYPES:
                raise TypeError
            self.dtype = array.dtype
        elif isinstance(array, ArrayConverter.SUPPORTED_NON_ARRAY_TYPES):
            self.array_type = np.ndarray
            self.is_num = True
            self.dtype = np.dtype(type(array))
        else:
            raise TypeError(f"Template type {self.array_type}" f" is not supported")

    def convert(self, input_array, target_type=None, target_array=None):
        """Convert input array to target data type.

        Args:
            input_array (tuple | list | np.ndarray | torch.Tensor | int | float ):
                Input array. Defaults to None.
            target_type (<class 'np.ndarray'> | <class 'torch.Tensor'>, optional):
        ...
        if target_type is not None:
            ...
            converted_array = (
                ...
                input_array.cpu().numpy().astype(np.float32)
                if target_type == np.ndarray
                # default dtype is float32, device is 'cpu'
                else torch.tensor(input_array, dtype=torch.float32)
            )
        else:
            ...
            converted_array = (
                ...
                else target_array.new_tensor(input_array)
            )
    ...
    def recover(self, input_array):
        ...
        if isinstance(input_array, self.array_type):
            return input_array
        converted_array = (
            input_array.cpu().numpy().astype(self.dtype)
            if isinstance(input_array, torch.Tensor)
            else torch.tensor(input_array, dtype=self.dtype, device=self.device)
        )
        if self.is_num:
