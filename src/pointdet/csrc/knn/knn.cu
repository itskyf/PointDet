...
__global__ void KNearestNeighborKernelV0(const scalar_t* __restrict__ points1,
                                         ...
                                         const size_t N, const size_t P1, const size_t P2,
                                         const size_t D, const size_t K, const size_t norm) {
...
__global__ void KNearestNeighborKernelV1(const scalar_t* __restrict__ points1,
                                         const scalar_t* __restrict__ points2,
                                         const int64_t* __restrict__ lengths1,
                                         const int64_t* __restrict__ lengths2,
                                         scalar_t* __restrict__ dists, int64_t* __restrict__ idxs,
                                         const size_t N, const size_t P1, const size_t P2,
                                         const size_t K, const size_t norm) {
...
struct KNearestNeighborV1Functor {
  static void run(size_t blocks, size_t threads, const scalar_t* __restrict__ points1,
                  const scalar_t* __restrict__ points2, const int64_t* __restrict__ lengths1,
                  const int64_t* __restrict__ lengths2, scalar_t* __restrict__ dists,
                  int64_t* __restrict__ idxs, const size_t N, const size_t P1, const size_t P2,
                  const size_t K, const size_t norm) {
...
__global__ void KNearestNeighborKernelV2(const scalar_t* __restrict__ points1,
                                         const scalar_t* __restrict__ points2,
                                         const int64_t* __restrict__ lengths1,
                                         const int64_t* __restrict__ lengths2,
                                         scalar_t* __restrict__ dists, int64_t* __restrict__ idxs,
                                         const int64_t N, const int64_t P1, const int64_t P2,
                                         const size_t norm) {
...
struct KNearestNeighborKernelV2Functor {
  static void run(size_t blocks, size_t threads, const scalar_t* __restrict__ points1,
                  const scalar_t* __restrict__ points2, const int64_t* __restrict__ lengths1,
                  const int64_t* __restrict__ lengths2, scalar_t* __restrict__ dists,
                  int64_t* __restrict__ idxs, const int64_t N, const int64_t P1, const int64_t P2,
                  const size_t norm) {
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
...
__global__ void KNearestNeighborKernelV3(const scalar_t* __restrict__ points1,
                                         const scalar_t* __restrict__ points2,
                                         const int64_t* __restrict__ lengths1,
                                         const int64_t* __restrict__ lengths2,
                                         scalar_t* __restrict__ dists, int64_t* __restrict__ idxs,
...
std::tuple<at::Tensor, at::Tensor> KNearestNeighborIdxCuda(
    ...
    const at::Tensor& lengths2, const int norm, const int K, int version) {
  ...
  if (version < 0) {
    ...
    std::cout << "WARNING: Requested KNN version " << version << " is not compatible with D = " << D
              << "; K = " << K << ". Falling back to version = " << new_version << std::endl;
    version = new_version;
  }
...
__global__ void KNearestNeighborBackwardKernel(const float* __restrict__ p1,          // (N, P1, D)
                                               ...
                                               const size_t N, const size_t P1, const size_t P2,
                                               const size_t K, const size_t D, const size_t norm) {
  ...
  for (size_t i = tid; i < N * P1 * K * D; i += stride) {
    ...
    if ((p1_idx < num1) && (k < num2)) {
      ...
      if (norm == 1) {
        float sign =
            (p1[n * P1 * D + p1_idx * D + d] > p2[n * P2 * D + p2_idx * D + d]) ? 1.0 : -1.0;
        diff = grad_dist * sign;
      } else {  // norm is 2
        diff =
            2.0 * grad_dist * (p1[n * P1 * D + p1_idx * D + d] - p2[n * P2 * D + p2_idx * D + d]);
      }
...
std::tuple<at::Tensor, at::Tensor> KNearestNeighborBackwardCuda(
    const at::Tensor& p1, const at::Tensor& p2, const at::Tensor& lengths1,
    const at::Tensor& lengths2, const at::Tensor& idxs, int norm, const at::Tensor& grad_dists) {
  // Check inputs are on the same device
  at::TensorArg p1_t{p1, "p1", 1}, p2_t{p2, "p2", 2}, lengths1_t{lengths1, "lengths1", 3},
  ...
  at::checkAllSameGPU(c, {p1_t, p2_t, lengths1_t, lengths2_t, idxs_t, grad_dists_t});
  ...
  TORCH_CHECK(N == p2.size(0), "Point sets must have the same batch dimension");
  TORCH_CHECK(p1.size(2) == D, "Point sets must have the same last dimension");
  ...
  KNearestNeighborBackwardKernel<<<blocks, threads, 0, stream>>>(
      p1.contiguous().data_ptr<float>(), p2.contiguous().data_ptr<float>(),
      lengths1.contiguous().data_ptr<int64_t>(), lengths2.contiguous().data_ptr<int64_t>(),
      idxs.contiguous().data_ptr<int64_t>(), grad_dists.contiguous().data_ptr<float>(),
      grad_p1.data_ptr<float>(), grad_p2.data_ptr<float>(), N, P1, P2, K, D, norm);
